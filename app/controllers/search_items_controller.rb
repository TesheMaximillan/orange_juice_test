class SearchItemsController < ApplicationController
  ...
  def index
    @search_items = SearchItem.all.where(user: User.find(params[:user_id])).order(rank: :desc)
  ...
  def create
    if search_item_exists?
      @search_item = SearchItem.all.where(user: params[:user_id]).where(text: search_item_params[:text]).first
      @search_item.update(rank: @search_item.rank + 1)
    else
      @search_item = SearchItem.new(search_item_params)
      @search_item.user_id = params[:user_id]
      @search_item.save
    end
  ...
  def search_item_exists?
    SearchItem.all.where(user: User.find(params[:user_id])).where(text: search_item_params[:text]).exists?
  end
end
